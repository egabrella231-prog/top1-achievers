...
const App: React.FC = () => {
  ...
  const [selectedLevel, setSelectedLevel] = useState<Level | null>(() => {
    try {
      const saved = localStorage.getItem('pocket_tutor_level');
      return saved ? saved as Level : null;
    } catch (e) {
      return null;
    }
  });
  const [selectedSubject, setSelectedSubject] = useState<SubjectOption | null>(() => {
    try {
      const saved = localStorage.getItem('pocket_tutor_subject');
      return saved ? JSON.parse(saved) : null;
    } catch (e) {
      return null;
    }
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    try {
      const saved = localStorage.getItem('pocket_tutor_messages');
      if (saved) {
  ...
  useEffect(() => {
    ...
    console.log("Pocket Tutor 2026 - Neural Link Status Check [v1.2]...");
    if (!apiKey || apiKey === 'undefined' || apiKey === 'null' || apiKey === '') {
      console.warn("CRITICAL: GEMINI_API_KEY is not detected in the environment. Ensure it is set in Vercel and you have REDEPLOYED.");
    } else {
      console.log("Neural Link: API Key detected (Starts with " + apiKey.substring(0, 4) + ").");
    }
  }, []);
  ...
  useEffect(() => {
    if (selectedLevel) localStorage.setItem('pocket_tutor_level', selectedLevel);
    else localStorage.removeItem('pocket_tutor_level');
  }, [selectedLevel]);

  useEffect(() => {
    if (selectedSubject) localStorage.setItem('pocket_tutor_subject', JSON.stringify(selectedSubject));
    else localStorage.removeItem('pocket_tutor_subject');
  }, [selectedSubject]);

  useEffect(() => {
    if (messages.length > 0) localStorage.setItem('pocket_tutor_messages', JSON.stringify(messages));
    else localStorage.removeItem('pocket_tutor_messages');
  }, [messages]);

  // Re-initialize chatRef on refresh if subject is selected
  useEffect(() => {
    if (selectedSubject && !chatRef.current) {
      try {
        chatRef.current = createChatInstance(selectedSubject.id, selectedSubject.level);
      } catch (e) {
        console.error("Failed to re-initialize chat node:", e);
      }
    }
  }, [selectedSubject]);

  // Ensure greeting if messages are empty but subject is selected
  useEffect(() => {
    if (selectedSubject && messages.length === 0 && currentUser) {
      setMessages([{ 
        id: 'g' + Date.now(), 
        role: 'assistant', 
        content: `Greetings, Scholar ${currentUser.name}. I am your ${selectedSubject.id} specialist for the 2026 ${selectedLevel} curriculum. What shall we explore today?`, 
        timestamp: new Date() 
      }]);
    }
  }, [selectedSubject, messages.length, currentUser, selectedLevel]);
  ...
  useEffect(() => {
    if (currentUser && currentUser.status === 'pending' && !currentUser.isAdmin) {
      fetchUsers();
      const interval = setInterval(fetchUsers, 30000); // Auto-check every 30s
  ...
  const startVoiceMode = async () => {
    ...
    try {
      ...
      if (err.message === "API_KEY_MISSING") {
        setStatusMessage("Neural Link Error: Gemini API Key is missing. Please set GEMINI_API_KEY in your Vercel Dashboard and REDEPLOY your app.");
      } else if (errorStr.includes("api_key_invalid") || errorStr.includes("invalid api key")) {
        setStatusMessage("Neural Link Error: The API Key provided is invalid. Please double-check it in Google AI Studio.");
      } else if (errorStr.includes("quota") || errorStr.includes("rate limit")) {
        setStatusMessage("Neural Link Error: API Quota or Rate Limit exceeded. Please try again later.");
      } else if (errorStr.includes("unsupported_location") || errorStr.includes("location not supported")) {
        setStatusMessage("Neural Link Error: Gemini API is not yet available in your current region.");
      } else {
  ...
  const selectSubject = (s: SubjectOption) => {
    ...
    try {
      ...
      let errorMessage = "Neural link disruption. Please retry.";
      const errorStr = error.message?.toLowerCase() || "";
      if (error.message === "API_KEY_MISSING") {
        errorMessage = "Neural Link Error: Gemini API Key is missing. Please set GEMINI_API_KEY in your Vercel Dashboard and REDEPLOY your app.";
      } else if (errorStr.includes("api_key_invalid") || errorStr.includes("invalid api key")) {
        errorMessage = "Neural Link Error: The API Key provided is invalid. Please double-check it in Google AI Studio.";
      } else if (errorStr.includes("quota") || errorStr.includes("rate limit")) {
        errorMessage = "Neural Link Error: API Quota or Rate Limit exceeded. Please try again later.";
      } else if (error.message) {
        errorMessage = `Neural Link Error: ${error.message}`;
      }
      setMessages([{ id: 'err', role: 'assistant', content: errorMessage, timestamp: new Date() }]);
    }
  };
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        if (error.message === "API_KEY_MISSING") {
          errorMessage = "Neural Link Error: Gemini API Key is missing. Please set GEMINI_API_KEY in your Vercel Dashboard and REDEPLOY your app.";
        } else if (errorStr.includes("api_key_invalid") || errorStr.includes("invalid api key") || errorStr.includes("403")) {
          errorMessage = "Neural Link Error: The API Key provided is invalid or restricted. Please double-check it in Google AI Studio and ensure the Gemini API is enabled.";
        } else if (errorStr.includes("quota") || errorStr.includes("rate limit") || errorStr.includes("429")) {
          errorMessage = "Neural Link Error: API Quota or Rate Limit exceeded. Please try again later or upgrade to a paid tier.";
        } else if (errorStr.includes("network") || errorStr.includes("fetch") || errorStr.includes("failed to fetch")) {
          errorMessage = "Neural Link Error: Network connection issue. Please check your internet, disable any VPNs/Adblockers, and retry.";
        } else if (errorStr.includes("unsupported_location") || errorStr.includes("location not supported")) {
          errorMessage = "Neural Link Error: Gemini API is not yet available in your current region.";
        } else if (errorStr.includes("safety") || errorStr.includes("blocked")) {
          errorMessage = "Neural Link Error: The response was blocked by safety filters. Please try rephrasing your question.";
        } else if (error.message) {
          errorMessage = `Neural Link Error: ${error.message}`;
        } else {
          errorMessage = `Neural Link Error: ${JSON.stringify(error)}`;
        }
      }
      
      setMessages(prev => [...prev, { id: 'err', role: 'assistant', content: errorMessage, timestamp: new Date() }]);
    } finally {
      setIsTyping(false);
    }
  };
          ...
          <div className={`glass rounded-[2.5rem] shadow-2xl p-10 border border-white ${authView === 'signup' ? 'flex flex-col md:flex-row gap-10 items-stretch' : ''}`}>
            ...
            <div className={authView === 'signup' ? 'flex-1 order-2 md:order-1' : 'w-full'}>
              ...
              <form onSubmit={authView === 'login' ? handleLogin : authView === 'signup' ? handleSignUp : handleAdminLogin} className="space-y-6">
                ...
                <button type="submit" disabled={isAuthenticating} className={`w-full bg-slate-950 text-white font-black py-5 rounded-2xl shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all uppercase tracking-[0.2em] text-[11px] ${isAuthenticating ? 'opacity-50 cursor-not-allowed' : ''}`}>
                  {isAuthenticating ? 'Processing...' : (authView === 'login' ? 'Initiate Access' : authView === 'signup' ? 'Request Enrollment' : 'Authorize Node')}
                </button>
              </form>
